...
$(function() {
    ...
    toast = factory(window.jQuery);
}(0, function($) {
    return toastId = 0, toastType_info = "info", toastr = {
        clear: function($toastElement, clearOptions) {
            var options = getOptions();
            $container || getContainer(options), clearToast($toastElement, options, clearOptions) || function(options) {
        ...
        remove: function($toastElement) {
            var options = getOptions();
            $container || getContainer(options), $toastElement && 0 === $(":focus", $toastElement).length ? removeToast($toastElement) : $container.children().length && $container.remove();
        },
        getContainer: getContainer,
        info: function(message, title, optionsOverride) {
            return console.log(toastr), function(map) {
                var options = getOptions(), iconClass = map.iconClass || options.iconClass;
                if (void 0 !== map.optionsOverride && (options = $.extend(options, map.optionsOverride), 
                iconClass = map.optionsOverride.iconClass || iconClass), !function(options, map) {
                    ...
                    function() {
                        if (map.title) {
                            ...
                            options.escapeHtml && (suffix = escapeHtml(map.title)), $titleElement.append(suffix).addClass(options.titleClass), 
                            $toastElement.append($titleElement);
                        }
                    }(), function() {
                        if (map.message) {
                            var suffix = map.message;
                            options.escapeHtml && (suffix = escapeHtml(map.message)), $messageElement.append(suffix).addClass(options.messageClass), 
                            $toastElement.append($messageElement);
                        }
                    }(), options.closeButton && ($closeElement.addClass(options.closeClass).attr("role", "button"), 
                    $toastElement.prepend($closeElement)), options.progressBar && ($progressElement.addClass(options.progressClass), 
                    $toastElement.prepend($progressElement)), options.rtl && $toastElement.addClass("rtl"), 
                    ...
                    }), 0 < options.timeOut && (intervalId = setTimeout(hideToast, options.timeOut), 
                    ...
                    options.closeOnHover && $toastElement.hover(stickAround, delayedHideToast), !options.onclick && options.tapToDismiss && $toastElement.click(hideToast), 
                    options.closeButton && $closeElement && $closeElement.click(function(event) {
                        event.stopPropagation ? event.stopPropagation() : void 0 !== event.cancelBubble && !0 !== event.cancelBubble && (event.cancelBubble = !0), 
                        options.onCloseClick && options.onCloseClick(event), hideToast(!0);
                ...
                function hideToast(override) {
                    var method = override && !1 !== options.closeMethod ? options.closeMethod : options.hideMethod, duration = override && !1 !== options.closeDuration ? options.closeDuration : options.hideDuration, easing = override && !1 !== options.closeEasing ? options.closeEasing : options.hideEasing;
                    if (!$(":focus", $toastElement).length || override) return clearTimeout(progressBar.intervalId), 
                    $toastElement[method]({
                        duration: duration,
                        easing: easing,
                        complete: function() {
                            removeToast($toastElement), clearTimeout(intervalId), options.onHidden && "hidden" !== response.state && options.onHidden(), 
                            response.state = "hidden", response.endTime = new Date(), publish(response);
                        }
                    });
                }
                function delayedHideToast() {
                    (0 < options.timeOut || 0 < options.extendedTimeOut) && (intervalId = setTimeout(hideToast, options.extendedTimeOut), 
    ...
    function getContainer(options, create) {
        return options = options || getOptions(), ($container = $("#" + options.containerId)).length || create && ($container = function(options) {
            return ($container = $("<div/>").attr("id", options.containerId).addClass(options.containerClass).addClass(options.positionClass)).appendTo($(options.target)), 
            $container;
        }(options)), $container;
    }
    ...
    var $container, listener, previousToast, toastId, toastType_info, toastr;
}), $(function() {
